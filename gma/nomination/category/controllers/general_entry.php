...
if($_SERVER['REQUEST_METHOD'] == 'POST') 
{
	...
	$contact1 = trim(strip_tags($_POST["contact1"]));
	$contact2 = trim(strip_tags($_POST["contact2"]));
	$contact3 = trim(strip_tags($_POST["contact3"]));
	$contact4 = trim(strip_tags($_POST["contact4"]));
	$contact5 = trim(strip_tags($_POST["contact5"]));

	$duplicates = 0; //$data_Obj->duplicates_check("hair_stylist", $nominee_name, $email, $mobile_number, $category); contact1

	// var_dump($duplicates);
	if (trim($duplicates) == 0) 
	{
		$result = $data_Obj->save_general_nomination($user_value, $category, $nominee_name, $email, $mobile_number, $nominee_bio, $instagram, $facebook, $other, $company_details, $comapny_bio, $company_instagram, $company_facebook, $company_other, $company_name, $phone_number, $company_email, $contact1, $contact2, $contact3, $contact4, $contact5);
		if (trim($result > 0)) {
