...
if($_SERVER['REQUEST_METHOD'] == 'POST') 
{
	...
	$editorial_project_name1 = htmlspecialchars(htmlentities(trim(strip_tags($_POST["editorial_project_name1"]))));
	$editorial_project_description1 = htmlspecialchars(htmlentities(trim(strip_tags($_POST["editorial_project_description1"]))));
	$editorial_project_name2 = htmlspecialchars(htmlentities(trim(strip_tags($_POST["editorial_project_name2"]))));
	$photo_link2 = htmlspecialchars(htmlentities(trim(strip_tags($_POST["photo_link2"]))));
	$editorial_project_description2 = htmlspecialchars(htmlentities(trim(strip_tags($_POST["editorial_project_description2"]))));
	$editorial_project_name3 = htmlspecialchars(htmlentities(trim(strip_tags($_POST["editorial_project_name3"]))));
	$photo_link3 = htmlspecialchars(htmlentities(trim(strip_tags($_POST["photo_link3"]))));
	$editorial_project_description3 = htmlspecialchars(htmlentities(trim(strip_tags($_POST["editorial_project_description3"]))));
	...
	{
		$result = $data_Obj->save_editorial_nomination($user_value, $category, $nominee_name, $email, $mobile_number, $nominee_bio, $instagram, $facebook, $other, $company_details, $comapny_bio, $company_instagram, $company_facebook, $company_other, $company_name, $phone_number, $company_email, $editorial_project_name1, $photo_link1, $editorial_project_description1, $editorial_project_name2, $photo_link2, $editorial_project_description2, $editorial_project_name3, $photo_link3, $editorial_project_description3);
		if (trim($result > 0)) {
